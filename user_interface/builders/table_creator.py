...
class TableCreator:
    ...
    def create_table() -> Table:
        ...
        return Table([TableCreator.create_grid(index, grid_image_mapper) for index in range(0, table_size)])

    @staticmethod
    def create_grid(index: int, grid_image_mapper: GridImageMapper) -> Grid:
        cell_position = TableCreator.get_grid_position(index)
        rect = pygame.Rect(cell_position, Grid.GRID_SIZE)
        return Grid(index=index, image=GridImage.CELL, rect=rect, grid_image_mapper=grid_image_mapper)
